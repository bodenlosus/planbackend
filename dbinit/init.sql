...
SELECT
  t.depot_id,
  ap.tstamp,
  SUM(COALESCE(t.running_amount, 0) * ap.close - COALESCE(t.running_commission, 0)) AS value
FROM
  api.asset_prices ap
  INNER JOIN LATERAL (
    SELECT
...
--     tstamp,
--     NTH_VALUE(cash + assets, 2) OVER (PARTITION BY depot_id ORDER BY tstamp DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS prev_1d_total,
--     NTH_VALUE(cash + assets, 23) OVER (PARTITION BY depot_id ORDER BY tstamp DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS prev_1m_total,
--     NTH_VALUE(cash + assets, 253) OVER (ORDER BY tstamp DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS prev_1y_total,
